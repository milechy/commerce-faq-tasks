...
// Worktree detection: skip writing to .wolf/* when running inside a git worktree.
// Reason: parallel writes from multiple worktrees cause EPERM/EEXIST on
// _session.json and token-ledger.json during 24h ループ並列実行.
// Decision rationale: docs/24H_AUTOMATION_R2C_GAP_ANALYSIS.md §3 (Option B採用).
try {
    ...
    // not a git repo or git unavailable — continue with normal behavior
}
async function main() {
    ensureWolfDir();
