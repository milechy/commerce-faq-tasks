...
## Security L1-L4 – Widget セキュリティ強化（完了）

- **L1: Widget難読化** (`SCRIPTS/build-widget-obfuscated.sh`) — JavaScript obfuscation, string encoding, self-defending
- **L2: SourceMap無効化** — `vite.config.ts` の `sourcemap: false`、console.log 除去
- **L3: APIドメインバインド** — `api_keys.allowed_origins` カラム追加、リクエスト元ドメイン検証
- **L4: 動的Widget配信** — テナント毎に `GET /widget.js?key=<apiKey>` で個別ウィジェット生成

Status: ✅ Completed (2026-03-24)

---

## Phase43 – フィードバック管理 + Admin AIチャットナビ（完了）

- **P1: フィードバック管理基盤** — `admin_feedback` テーブル + CRUD API + Super Admin UI
- **P1: 管理画面サポートAI チャットナビ** — Admin画面内浮きFAB、super_admin限定
- **P2: AIサポートのインテント振り分け + RAG統合** — `business_faq` インテント → `faq_docs` 直接クエリ
- **P2: 日本語キーワード抽出** — N-gram / 漢字連続抽出方式
- **P2: ソース注釈表示** — business_faq回答にインライン注釈

Status: ✅ Completed (2026-03-24)

---

## Phase44 – デフォルトアバター管理（完了）

- `default_avatar_configs` テーブル + シードスクリプト（8体のプリセットアバター）
- Super Admin UI: デフォルトアバター一覧・編集・有効/無効切り替え
- テナント側でデフォルトアバターを選択できる仕組み（`avatar_configs.is_default`）
- Widget: アバターFABにアイドルモーション追加（接続前のループアニメーション）
- Widget obfuscation を Phase44 ビルドに統合
