...
**取れることと戻せることは別なので、両方を確認している。** 復旧確認は
`bash SCRIPTS/backup-postgres.sh --restore-test` で繰り返せる（検証用DB
`r2c_restore_test` へ戻し、テーブル数を本番と突き合わせてから削除する。本番には書き込まない）。
**一度きりにしないこと** — バックアップは静かに腐る。

> **この表は 2026-08-18 まで誤っていた。** 「`pg_dump` (VPS cron) / 日次 / 7日」と
> 実装済みであるかのように書きながら、実際には1本も取られていなかった
> （`crontab -l` は avatar-agent 監視のみ、`/backup/` はディレクトリ自体が無かった）。
> 下の §4.2 が同じ cron を「想定」と書いており、ドキュメント内で矛盾していた。
>
> **「バックアップがある」という誤記は、無いことより危険。** 障害時に復旧できると誤認したまま
> 判断してしまう。この表を書き換えるときは、必ず実測してから直すこと。
