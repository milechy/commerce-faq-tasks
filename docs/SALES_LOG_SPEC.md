# Sales Log Specification (Phase14)
...
- 既存の ClarifyLog（/integrations/notion/clarify-log）を一般化し、**1つの仕組みで SalesFlow 全体を記録可能**にする。
...
| `templateId` | string? | Notion テンプレのページID（fallback の場合 null）|
...
### dialogAgent
- `runSalesOrchestrator` の結果で `nextStage` がセットされ、`prompt`（テンプレ）が返ってきた場合：
  - 回答をユーザーへ返す前に SalesLogWriter に書き込む。
  - Clarify の場合は ClarifyLogWriter を一般化したものを利用。
...
**Phase14 の目標：SalesFlow 全体の観測と最適化ができる基盤を完成させること。**
