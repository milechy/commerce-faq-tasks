...
### 13. ターン跨ぎ untrusted-read ラッチが正規表現ヒューリスティックで解除される

`agentRoutes.ts:871-872` の `untrustedReadLatchStore` は**プロセス内 `Map`**（TTL 30分）。
単一プロセスであることは `ecosystem.config.cjs:26` の `instances: 1` という**デプロイ規約**に
依存しており、コード側の不変条件ではない（禁止30 と同根）。再起動・多重起動で消える。

解除判定は `agentRoutes.ts:907-908` の操作動詞マッチのみ:

    /(削除|消して|消去|削って|破棄|送信|返信|送って|実行|公開|非公開|停止|再開|有効化|
      無効化|更新|変更|上書き|反映|保存|登録|設定して|直して|修正して|依頼して|リセット|
      対応完了|完了にして|承認|確定)/

**店主が顧客の文面を引用しただけでこれらの語を含めばバイパスが成立しうる。**
`:900-906` のコメント自身がヒューリスティックであることを自認している。

### 14. 連鎖ブロックの2集合が手動列挙で、網羅を守る機械テストが無い

`SUGGEST_TO_SAVE_TOOL`（`agentRoutes.ts:785-798`）と `UNTRUSTED_TEXT_READ_TOOLS`（`:831-838`、6本）。
`:840-846` が「列挙方式は残り、登録漏れの再発余地も残る。実施する場合は別タスクとして起票」と
明記したまま**恒久対応が未着手**。

過去の実測発覚が3件:
- `suggest_avatar_preset` / `suggest_category_persona` の登録漏れ（`:792-798`）
- `get_conversation_evaluation` / `suggest_faq_import_from_urls` の未収録（`:820-828`）
- Record が1キー1値しか持てず「**32ツール中7ツールにしか効いていなかった**」（`:806-812`）

`confirmPolicy.test.ts` はこの2集合を検査対象にしていない。
...
| 1・13・14（設計） | [P2] 管理AIの承認を「サーバが保持する承認マーク」にする | `1218170825242147` |
...
| 5・6・10・11・15 | [P3] 監査で判明した小さい是正と、CLAUDE.md の失効記述の訂正 | `1218170942549672` |
