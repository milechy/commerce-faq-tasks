...
```bash
claude mcp add --scope project mobile-mcp npx @mobilenext/mobile-mcp@latest
```
...
# 出力例: mobile-mcp: npx @mobilenext/mobile-mcp@latest (connected)
```

### 実行方法

Mobile MCP を使って widget.js を 390px 相当の viewport で動作確認する。
mobilenext.ai はローカル実機またはシミュレーター（Xcode iOS / Android SDK）経由の制御ツール。
実機/シミュレーターがない場合は `tests/e2e/widget-mobile.spec.ts`（Playwright iPhone 12 エミュレーション）で代替可能。
