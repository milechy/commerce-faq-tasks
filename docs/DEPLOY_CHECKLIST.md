...
| `src/api/admin/tenants/migration_excluded_page_patterns.sql` | 許可ドメイン内でもウィジェットを非表示にするページ除外設定(`tenants.excluded_page_patterns TEXT[]`)追加(`ADD COLUMN IF NOT EXISTS`、既定`'{}'`)。**★migration → デプロイの順。逆順は widget.js 配信が全テナントで500になる** — `src/api/widget/routes.ts` の `GET /widget/:tenantSlug.js` が `SELECT ... excluded_page_patterns FROM tenants` をフォールバック無しで発行しており、列が無いと42703が try/catch経由でそのまま500になる(該当テナントだけでなく全ウィジェット埋め込み先が影響を受ける)。同様に `GET /v1/admin/my-tenant`・`GET /v1/admin/tenants/:id`・CopilotUIツール(`get_tenant_settings`/`update_excluded_page_patterns`, `src/api/admin/agent/actionExecutor.ts`)も同じ列をフォールバック無しでSELECTしており、未適用のまま新コードを配備すると同時に500化する | ✅ 2026-09-03 適用済み（本番実測: ARRAY / NOT NULL / default '{}'） |
| `src/migrations/phase78_wp_provisionings.sql` | WordPress プラグインのセルフサインアップにおける確定前の中間状態テーブル(`wp_provisionings`)を新設 + 部分ユニークインデックス`uq_wp_provisionings_provisioned_site`(同一 origin で2つ目のテナントを作らせない。プラグインを削除して再インストールすると wp_options が消え、利用者は同じドメインから何度でも接続をやり直すため、アプリ側の存在チェックだけでは同時実行で二重発行され得る)。**★migration → デプロイの順**。ただし未適用でも既存機能は壊れない — このテーブルを参照するのは新設の `/v1/public/wp/*` だけで、既存の配信・課金・管理経路は一切触らない。未適用のまま配備した場合は 42P01 でプロビジョニングAPIのみが失敗し、WordPress プラグインからの新規接続が通らない(既存テナントのウィジェットには影響しない) | ⬜ 未適用 |
| `src/migrations/phase79_tenants_provisioning_source.sql` | `tenants.provisioning_source`(`TEXT NOT NULL DEFAULT 'manual'`, CHECK `manual`/`wordpress_plugin`)を追加。Super Adminでプラグイン経由テナントを識別できるようにする(WP-15 / D11 / §13.5)。未適用でも既存機能は壊れない(`DEFAULT 'manual'` があるため既存の手動作成INSERTはこの列を意識しない)が、`src/api/widget/wpProvisionRoutes.ts` の `completeWpProvisioning` は明示INSERTするため、未適用のまま配備すると **WordPress プラグイン経由のテナント発行のみ** 42703 で失敗する(既存の手動発行・配信経路には影響しない) | ⬜ 未適用 |
| `src/api/admin/tenants/migration_free_ad_plan.sql` | `tenants.plan` の CHECK 制約に `free_ad` を追加。**このチェックリストに一度も登録されていなかったのを WP プラグイン計画(WP-1)着手時に発見**。適用状況は本ドキュメントからは確認できない — psql で `\d tenants` を実行し、`tenants_plan_check` に `free_ad` が含まれているかを直接確認すること。**未適用のまま `plan='free_ad'` を INSERT/UPDATE すると DB エラー(制約違反)になる**（アプリ側の zod バリデーションは通過するため、ここで初めて気づく形になる。CLAUDE.md 禁止55と同型）。WP-1(WordPressプラグインのセルフサインアップ)は plan='free_ad' で直接 INSERT するため、**未適用のままではプロビジョニングAPIが必ず失敗する** | ⬜ 状態不明・要実機確認 |
| `src/api/admin/tenants/migration_standard_plan.sql` | `tenants.plan` の CHECK 制約に `standard` を追加。同じくチェックリスト未登録だったため今回まとめて記載。`migration_free_ad_plan.sql` の**後に**適用する必要がある（DROP + ADD で制約を丸ごと置き換える方式のため、順序を逆にすると `free_ad` が制約から落ちる）。Standard プランの機能（既定アバター利用等）は既に本番で使われている記述が随所にあるため、実際には適用済みの可能性が高いが、**このチェックリストでは追跡されていなかった** | ⬜ 状態不明・要実機確認 |
