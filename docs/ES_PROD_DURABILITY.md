...
> 以下のファイルはリポジトリのルートに配置済み。cutover 時は VPS の作業ディレクトリ（例: `/opt/rajiuce-infra/es/`）にコピーして使用する（§4 手順 1 参照）。

### 3.1 `Dockerfile.es`（kuromoji 焼き込み）

**ファイル:** リポジトリ root `Dockerfile.es`
...
**ファイル:** リポジトリ root `docker-compose.prod-es.yml`
...
# 1. Dockerfile.es / docker-compose.prod-es.yml をリポジトリから VPS 作業ディレクトリへ転送
#    （ローカルからリポジトリ内ファイルを scp する例）
mkdir -p /opt/rajiuce-infra/es
scp Dockerfile.es docker-compose.prod-es.yml <vps-host>:/opt/rajiuce-infra/es/
...
1. ✅ **`sync-es.ts` を delete-first から alias swap 方式へ（完了: 2026-06-06）**
   - `SCRIPTS/sync-es.ts` を alias-swap 方式に変更済み（このブランチ同梱）。
   - `faq_<tenant>_<ts>` に新規作成 → 全件 bulk 成功 → alias `faq_<tenant>` を原子的に張り替え → 旧 index 削除。
   - 途中失敗時は新 index のみ掃除し、現行 index/alias を無傷で温存する。
