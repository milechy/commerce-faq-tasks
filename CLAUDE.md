...
- **WordPress プラグイン（`r2c-ai-concierge`）は、この製品の 3 つ目の面ではなく、①の導入経路である。**
  プラグインは薄いラッパーに徹し、ウィジェット実装・プラン判定・ブランディングはすべてサービス側に置く。
  これは設計の好みではなく、wordpress.org ガイドライン #5（機能ロック禁止）と #10（クレジット表示は
  サービス側の出力なら可）を**同時に満たす唯一の構造**である。プラグイン側に持たせた瞬間に掲載が危うくなる。
  コードは別リポジトリ `milechy/r2c-wordpress-plugin`（GPLv2）。本リポジトリが持つのはサーバ側だけ。
  v1.0 のスコープはウィジェット設置＋設定のみ（記事の知識源同期・WooCommerce 連携は v1.1 以降）。
  要件・制約・テスト観点の一次資料は `docs/WORDPRESS_PLUGIN_REQUIREMENTS.md`。**着手前に必ず読む。**
...
- **価格は確定している。コードから導かない。** 2026-08-26 に価格を確定した
  ...
  いずれも年払いは 2ヶ月無料）。**課金単位はテキストが「会話」、アバターが「分」**（→ 禁止 56）。
...
| WordPress プラグイン本体（PHP） | **別リポジトリ `milechy/r2c-wordpress-plugin`（GPLv2）**。本リポジトリに置かない（ライセンス境界が曖昧になる） |
| WP 向け公開プロビジョニング API | `src/api/widget/` 配下に `register*Routes(app, db)` の形で追加し、`src/index.ts` の既存の登録列に並べる。`/v1/public/*` は現在1本も無いため新設になるが、認証層・レート制限・エラー形式は既存の作法に揃える |
...
## 接客エージェント（Layer 0〜3）の不変ルール

目的は「答える AI」ではなく **買う気にさせる店員**。Judge の4軸
（`psychology_fit` / `customer_reaction` / `stage_progress` / `taboo_violation`）が測るのは
解決率ではなく心理的適合と商談段階の前進であり、**設置箇所と評価軸が一致していなければ
評価値は改善の指針にならない**。要件・成功の定義・受け入れ条件は
`docs/SALES_AGENT_REQUIREMENTS.md` が正本（本節は要約。数値はそちらを見る）。

**層は順序ゲートで進む。前の層が通るまで次の層に着手しない。** 並行着手はしない（体制は実質1人）。

| 層 | やること | 次に進む条件 |
|---|---|---|
| Layer 0 | 痛みのある面にだけ widget を出し会話を成立させる／承認 UI を作り直す | 実テナント10社・月500会話・4往復以上20% |
| Layer 1 | テナントの「売れる店員」を Person Profile 化し、態度として常時注入する | 3社が自分で編集・承認している |
| Layer 3 | 1カート × 1アクション × 人間承認つきで基幹系に書き込む | 3社・月100件・成功率95% |
| Layer 2 | 実行イベントを成果台帳の唯一の真実にする | 10社が月次で参照している |

**置き場所（`## 実装の置き場所` の追加分）**

| やりたいこと | 置き場所 |
|---|---|
| widget をどの面に出すか | `src/api/widget/widgetGenerator.ts` の設定注入。**テナント側の貼り方に依存させない** |
| 書き込み実行の資格情報・lease・監査 | `hermes-vault`（自作しない。変更系は既定 OFF で導入） |
| 失敗の検出と「何を改善すべきか」の信号 | `hermes-dojo`（自作しない） |
| 「売れる店員」の蒸留 | `distilly`（自作しない）。出力は**方針と文体の層**であり事実の格納場所にしない |

**外部部品（Hermes プラグイン / スキル）の採否**

★の数と README は動作を保証しない。★446 のリポジトリに、ライフサイクルフックを1つも登録しない
「テレメトリ」プラグインが同梱されていた実例がある。**selfcheck を書き、意図的に壊して NG になることを
確認してから入れる。** 読み取りから始め、変更系は既定 OFF のまま導入する。
...
56. **アバターを「回数」で課金する／テキストを「リクエスト」で課金する。**
    **アバターの原価は回数ではなく時間に比例する。** 本番実測（90日）では 1 セッションあたり
    1 分未満が 819 件（平均原価 ¥19）に対し、**15 分以上が 72 件（7.6%）で平均原価 ¥799 と 42 倍の開き**があり、
    回数あたりの定額では長時間セッション 1 件で赤字になる。実測 **¥25.9/分**。
    `anam_session` が既に `CEIL(anam_session_seconds/60.0)` で分換算しているのと同じ扱いを全アバターに適用する。
    **テキストは逆に「会話（セッション）」で数える。** リクエスト単位にすると、
    会話を長く良くするという製品目標がそのままテナントの値上げになり、改善が摩擦に変わる。
    原価は ¥0.11/往復に対し ¥20/会話なので **1 会話 180 往復まで赤字にならない**。
    業界標準（Intercom・Zendesk＝解決単位、Tidio＝会話単位、KARAKURI＝件単位）とも、LP の「対話」表記とも一致する。

57. **層の順序ゲートを飛ばして着手する。**
    Layer 0（会話が成立しているか）が通らないうちに書き込み実行・横断学習・プロトコル対応に着手すると、
    誰も来ないドアに鍵をかける作業になる。**90日で実テナント10社・月500会話・4往復以上20%**に
    届かないなら、直すのは機能ではなく「AI 販売員」というカテゴリー定義そのもの。
    Judge の評価下限（`DEFAULT_MIN_MESSAGE_COUNT = 4`）を下げて数字を作るのは欠陥の隠蔽（→ 禁止43）。
58. **widget を全ページに出す／面の限定をテナント側の貼り方に依存させる。**
    訪問者に痛みが無い場所に出すと会話は成立しない（90日13件・平均1.5往復・4往復以上ゼロの実測）。
    出すのは**テナントごとに定義した「痛みのある面」**に限定し、
    その限定は**R2C 側の設定で効かせる**。テナントが埋め込みコードを全ページに貼っても崩れないこと。
    面ごとの「最初の一言」は静的コピーでよいが、**それ以降の応答を静的テンプレートで返す経路は作らない**
    （→ 禁止46）。
    **面を EC の3面（カゴ落ち・定期解約・配送遅延）に固定しない。** それは EC の場合の例であって
    定義ではない。実在テナントに EC でないもの（B2B のサービスサイト等）が含まれる以上、
    面は `page_url_match` / `exit_intent` / `idle_time` / `scroll_depth`（TriggerEngine が既に持つ4種）
    の組み合わせでテナントごとに表現する。詳細は `docs/SALES_AGENT_REQUIREMENTS.md` §5-1。
59. **外部部品を README と ★ の数で採用する。**
    「入れたのに何も記録しない」は実際に起きている。selfcheck を書き、フックを外す・引数を丸ごと出す等の
    **意図的な破壊で NG になることを確認**してから本番に入れる。依存を増やすほど
    「壊れたとき誰が直すか」が積み上がる（バス係数1）。
60. **Hermes をテナントのホットパス（回答生成・外部公開）に置く。**
    upstream に未修正のテナント分離欠落が2件ある。A2A の `contextId` はファイル名サニタイズで衝突し
    （呼び出し側が自由に決められる値で他テナントの履歴が読める）、単一ゲートウェイは全ユーザーを
    1つの `HERMES_HOME` で動かすためワークスペース（skill・cron・メモリ）が共有される。
    さらに A2A の `tenant` フィールドは `DESIGN.md` で "Deliberately out of scope" と明記されており、
    **プロトコル側の分離機構は使えない**。Hermes は社内運用の横断コーチに留め、
    外部に開くならテナント ID を強制注入するゲートウェイを前段に置く。
61. **取り消せない書き込みを最初のスコープに入れる／承認時点のプランで実行可否を判定する。**
    取り消し手段が無い操作は Layer 3 の初期スコープに入れない（「30分以内なら取り消せます」を先に見せる）。
    プランは**実行時点**で判定する（承認からダウングレードまでの間に実行され得る）。
    実行前に対象の現在状態を再取得する（別チャネルで既に解約済みのものを盲目的に実行しない）。
62. **実在の人物を素材にしたプロファイルを、同意と退職後の扱いを決めずに作る。**
    「売れる店員」の蒸留は実在の従業員が対象。本人同意・利用範囲・退職後の profile の扱いを
    先に決める。承認済みでも防御層（L5/L6）は素通ししない（→ 禁止33）。
63. **WordPress プラグインに、サービス側が持つべきものを持たせる。**
    プラグインは配布物であり、wordpress.org の審査対象である。以下はいずれも**掲載削除の理由**になる。
    - **Powered by バッジ・広告帯をプラグイン側で出力する。** ガイドライン #10 の但し書き
      「サービス側の出力ならブランディング可」に乗っているため、描画は `widget.js` にしか置けない。
      プラグインに1行でも入れた時点で違反になる（`free_ad` の広告帯も同じ）。
    - **支払い状態で分岐する機能ロックをプラグインのコードに置く。** #5（トライアルウェア禁止）。
      プラン差はサービス側（`planFeatures.ts` / `widgetGenerator.ts`）でのみ表現する。
    - **未接続状態で外部通信する。** #7。接続は同意チェック＋ボタン押下でのみ発火させる。
      SaaS 例外（登録・設定行為が同意とみなされる）は、この構造があって初めて成立する。
    - **設定画面を `admin.r2c.biz` の iframe にする。** #8 が admin ページの iframe を名指しで禁止。REST API で組む。
    - **静的版（`public/widget.js` + `data-tenant`）へ流す。** プラン判定を経由しない fail-open 経路（→ 38）。
      プラグインは動的版のみを使う。反映は最大 5 分遅れる（`max-age=300`）ので「即時反映」と読める文言を書かない。
    - **総量ガード無しで公開する。** プラグイン公開は機能追加ではなく、**無料テナントの自動増殖経路の開通**
      であり原価構造の変更にあたる（→ 39）。テナント単位の上限（`planQuota.ts`）は既にあるが、
      **テナント数そのものの上限は無い**。ガードは公開と同時に稼働していること。後から足す設計にしない。
    - **サイト所有証明を省いてキーを発行する。** 自己申告の URL だけで発行すると、第三者が他人のドメインで
      無制限にテナントを作れる。プラグインが自サイトに立てた REST ルートへのトークン照合＋メール到達確認の二段を通す。
...
- **WordPress プラグインは「利用者が壊す手順」を先に書く。** 想定内の正常系より、以下の 4 つで事故る。
  ①**ドメイン変更**（独自ドメイン移行・`http`→`https`・www 有無）で `allowed_origins` が古くなり
  **ウィジェットが無言で止まる** ②**プラグイン削除→再インストール**で option が消えた後に再接続し、
  同じドメインで**2 つ目のテナントを作る** ③本番サイトの**複製（ステージング）**で同じキーが 2 ドメインに乗る
  ④テーマに残った**手貼り `<script>` との二重表示**。いずれも「動かない理由が利用者に分かること」まで含めて固定する。
- **未接続状態で外部通信が 0 件であることをテストで固定する。** ガイドライン #7 の SaaS 例外が成立する前提そのもの。
  「接続していないのだから通信しないはず」は実装で崩れる。
- **R2C API 停止中にサイトのフロントが無傷であることを書く。** スクリプトタグの出力が保存済み設定のみに依存し、
  リクエスト時に R2C へ問い合わせないこと。テナントのサイトを落とすのが最悪の失敗。
...
**接客エージェント（Layer 0〜3）で最低限見るもの**

- **面の分離は件数ではなく性質で検査する。** 既存の台帳テストは行数固定でドリフトを検知できない
- **テナント分離は実 Postgres で検証する。** SQL 側と JS 側の判定がズレた前例がある
- **外部部品は「壊して NG になること」まで確認する。** 通ることの確認だけでは入れた意味を保証できない
- **承認が本番に届くところまで見る。** `is_active=true` になった提案が、
...
- **実行系（書き込み）の失敗文言には、業務状態がどうなっているかを必ず含める。**
  「エラーが発生しました」だけだと、店舗側が二重に手作業で処理する。
  *実行されたのか／されていないのか*を明示し、次に誰が何をするかを書く。
  成功時は取り消し可能な時間を同時に出す（「30分以内なら取り消せます」）。
- **WordPress プラグイン側の名前は `r2c_` プレフィックスで固定する。** 関数・クラス・option 名・
  REST ルート名すべて（localStorage キーの `r2c_` と同じ思想。WP はグローバル名前空間が共有のため衝突が実害になる）。
  slug は `r2c-ai-concierge` で確定しており**後から変更できない**。
- **WP 向け公開 API のエラーは、既存の `{ error: "snake_case", message: "日本語" }` 形式に揃える。**
  ステータスの意味は潰さない（→ 21）。403（サイト未検証）/ 404（トークン不明）/ 409（同一ドメインに既存テナント）/
