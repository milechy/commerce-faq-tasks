...
実装完了
  ...
  → git commit + push（ここまでCLI自動）
────────────────────────────────
以降は人間が実行:
  → Gate 4: claude --chrome でブラウザテスト（UI変更時）
  → デプロイ: bash SCRIPTS/deploy-vps.sh
  → DBマイグレーション: VPSでSQL手動実行（あれば）
  → Gate 5: curl https://api.r2c.biz/health + Admin UIログイン確認
  → Gate 6: UI調査（UI変更時のみ・Claude in Chrome）
  → Asanaタスク完了 + ドキュメント更新
```

### Codex Review 運用ルール
- review gate: **常時OFF**（自動ループはコスト消費が大きすぎる）
- 通常レビュー: PR前に1回だけ `/codex:review --base main --background`
- セキュリティ変更時のみ: `/codex:adversarial-review --background`
