    ...
    it('暦週(月曜00:00 JST起点)のレンジでクエリが組まれる', async () => {
      ...
      try {
        ...
        mockQuery
          ...
          .mockResolvedValueOnce({ rows: [{ n: 3 }] }) // FAQ総数(このテストの主眼ではないがPromise.allの一員として発火する)
          .mockResolvedValueOnce({ rows: [{ n: 2 }] }) // 公開FAQ数
          .mockResolvedValueOnce({ rows: [{ max: '2026-07-01T00:00:00.000Z' }] }); // FAQ最終更新日
