...
const SYSTEM_PROMPT = `あなたは書籍コンテンツのFAQ化アシスタントです。
...
function parseStructuredResponse(raw: string): Omit<StructuredChunk, "chunkIndex" | "pageNumber" | "originalText"> | null {
  // JSON ブロックを抽出
  ...
  try {
    ...
    return { category, summary, keywords, question, answer, confidence };
  } catch {
    return null;
  }
}
...
/**
 * TextChunk 配列を Groq 8b で 6 フィールド構造化する。
