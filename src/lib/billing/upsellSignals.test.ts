...
describe('computeUpsellSignals', () => {
  ...
  it('★出力に金額を表すキーが1つも無い★', () => {
    const r = computeUpsellSignals({ plan: 'standard', textUnits: 2000, avatarMinutes: 60 });
  ...
  it('込み枠を超過すると text_overage / avatar_overage が立つ', () => {
    const r = computeUpsellSignals({
      ...
      avatarMinutes: STD.avatarMinutes + 5,
    });
  ...
  it('超過していなくても8割で text_near_limit（超過してから気づかせない）', () => {
    const r = computeUpsellSignals({
      plan: 'standard', textUnits: Math.ceil(STD.textConversations * 0.8), avatarMinutes: 0,
    });
  ...
  it('7割では text_near_limit を出さない（過剰にプランを上げさせない）', () => {
    const r = computeUpsellSignals({
      plan: 'standard', textUnits: Math.floor(STD.textConversations * 0.7), avatarMinutes: 0,
    });
  ...
  it('超過済みなら near_limit は重ねない（同じことを2回言わない）', () => {
    const r = computeUpsellSignals({
      plan: 'standard', textUnits: STD.textConversations + 1, avatarMinutes: 0,
    });
    expect(r.signals).toContain('text_overage');
    expect(r.signals).not.toContain('text_near_limit');
  });
  ...
  it('★込み枠が無いプランの消化率は null（0% にしない）★', () => {
    const r = computeUpsellSignals({ plan: 'starter', textUnits: 100, avatarMinutes: 0 });
  ...
  it('Starter の上限到達で starter_cap_reached', () => {
    const r = computeUpsellSignals({
      plan: 'starter', textUnits: STARTER_MONTHLY_BILLED_QUANTITY_CAP, avatarMinutes: 0,
    });
  ...
  it('free_ad の上限到達で free_ad_limit_reached', () => {
    const r = computeUpsellSignals({
      plan: 'free_ad', textUnits: FREE_AD_MONTHLY_CONVERSATION_LIMIT, avatarMinutes: 0,
    });
  ...
  it('Growth の大規模利用で enterprise_nudge', () => {
    const r = computeUpsellSignals({
      plan: 'growth', textUnits: GROWTH_TEXT_UNITS_ENTERPRISE_NUDGE_THRESHOLD, avatarMinutes: 0,
    });
  ...
  it('上位プランでの消化率を返す（上げれば収まるかを金額抜きで判断できる）', () => {
    const r = computeUpsellSignals({
      plan: 'standard', textUnits: 1500, avatarMinutes: 0,
    });
  ...
  it('enterprise は次が無く、込み枠も無いので上位消化率は null', () => {
    const r = computeUpsellSignals({ plan: 'enterprise', textUnits: 10_000, avatarMinutes: 0 });
    expect(r.nextPlanCandidate).toBeNull();
    expect(r.utilizationPctOnNextPlan.text).toBeNull();
  });

  it('未知プラン / null でも落ちない', () => {
    expect(() => computeUpsellSignals({ plan: null, textUnits: 10, avatarMinutes: 0 })).not.toThrow();
    expect(() => computeUpsellSignals({ plan: 'zzz', textUnits: 10, avatarMinutes: 0 })).not.toThrow();
    expect(computeUpsellSignals({ plan: null, textUnits: 10, avatarMinutes: 0 }).nextPlanCandidate).toBeNull();
  });

  it('利用が無ければシグナルは空（何も言わない）', () => {
    expect(computeUpsellSignals({ plan: 'standard', textUnits: 0, avatarMinutes: 0 }).signals).toEqual([]);
  });
});
