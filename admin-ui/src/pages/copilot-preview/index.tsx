...
const REAL_TOOL_LABEL: Record<string, string> = {
  ...
  update_excluded_page_patterns: "Widgetを表示しないページの設定変更",
...
export default function CopilotPreviewPage() {
  ...
  const runOnboardingAwareBriefing = async (
    ...
    if (stage && scopedTenantId && !resolveShare(learningFeatures)) {
      push(say(
        "🌐 他社が磨いたAIの接客力を、そのまま貴社にも取り込みませんか？\n全国の参加テナントの会話から見つかった「効果的な言い回し」が「グローバルルール」として、貴社が何もしなくても自動でAIに反映されます。自社の会話データだけでAIを育てるより、はるかに早く賢くなります。参加は無料。共有されるのは匿名化された会話ログ・行動データのみで、金額情報は含まれず、いつでもOFFに戻せます。",
        [
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          {quota.freeAd ? (
            <>
              ...
              {quota.freeAd.adminUsed !== undefined && quota.freeAd.adminLimit !== undefined && (
            ...
            <>
              ...
              {quota.admin != null && quota.admin.included !== null && (
                <BillingQuotaBar label="管理AIへのご相談" used={quota.admin.used} included={quota.admin.included} unit="件" />
              )}
              {(quota.text.overage > 0 || quota.avatar.overageMinutes > 0 || (quota.admin?.overage ?? 0) > 0) && (
