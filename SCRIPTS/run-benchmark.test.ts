...
import { getFeatureFlags, formatResultsAsCsv } from './run-benchmark';
import type { ConversationResult, TestConversation } from './run-benchmark';
...
describe('Feature Flag切り替え', () => {
  ...
  it('未知の条件はエラーをthrowする', () => {
    expect(() => getFeatureFlags('Z')).toThrow('Unknown benchmark condition: Z');
  });
});
